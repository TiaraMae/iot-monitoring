...
- **Clean versions created:** `_Clean` folders for all 6 firmware variants (v1, v2 standard, v2 auto, v3, v4, gas dryer test) with placeholder credentials:
  - `YOUR_WIFI_SSID`, `YOUR_WIFI_PASSWORD`
  - `YOUR_MQTT_BROKER`, `YOUR_MQTT_USERNAME`, `YOUR_MQTT_PASSWORD`
- **v4 backend sanitized:** Hardcoded MQTT defaults removed from `iot_thesis_v4/app.py`. `MQTT_HOST`, `MQTT_USER`, `MQTT_PASS` now require environment variables with validation errors if missing.

### 2026-05-26 — Atmospheric Pressure Simplification
- **EMA removed:** `_update_atmospheric_pressure()` no longer uses exponential moving average.
- **Simple baseline update:** During idle, baseline updates to latest raw pressure reading every 2 minutes. 15-second settling guard rejects readings too close to running transitions.
- **Null-safe:** Guards against `None` raw_pressure. Hard reset on first idle after cycle end.
- **DB persistence:** Baseline stored in `appliances.atmospheric_pressure` and reloaded on backend restart.
...
- **Flask-Limiter** added (`flask-limiter>=3.5.0`): login 30/min, signup 5/min, default 200/min.
- **Session hardening:** `SESSION_COOKIE_SECURE=True`, `SESSION_COOKIE_HTTPONLY=True`, `SESSION_COOKIE_SAMESITE='Lax'`, `PERMANENT_SESSION_LIFETIME=12h`.
- **Null-safe sensor handling:** Missing DHT/BME readings emit `null` in telemetry; backend gracefully handles `None` values without crashing.

### 2026-05-26 — Dryer Incomplete Drying Alert Fix (3 Parts)
- **Root cause:** With "running only" firmware logic, the ESP32 stops sending MQTT messages when current drops below ~0.25A. The old code only called `check_fault_alerts()` when `final_amps >= 0.25`, so the `current < 0.15` cycle-end check was never reached. Gap-based finalization required a new message to arrive — if the cycle was the last of the day, no message arrived and the alert was never evaluated.
- **Part A — Global timeout check:** After every MQTT message insertion, ALL in-cycle dryers are scanned. If any have been idle >120s, `_finalize_dryer_cycle()` runs immediately. Fixes "last cycle of the day" missing alerts.
- **Part B — Fault checks on all dryer messages:** `check_fault_alerts()` is now called for ALL dryer messages (running + idle), not just `final_amps >= 0.25`. Required for upcoming "keep on sending data" firmware switch.
- **Part C — Guard real-time checks during idle:** Spike state machine and belt snap detection only run when `current >= 0.25`. Idle messages update cycle stats (last_time, rh_history, motor_readings) but reset `consecutive_below_lcl` to prevent false belt snap alerts.

### 2026-05-26 — Gauge Pressure UI Layout Fix
- **Dryer latest-data grid:** Changed from `grid-template-columns: repeat(auto-fill, minmax(130px, 1fr))` to `repeat(3, 1fr)` for dryers. Forces 5 items into 2 clean rows:
  - Row 1: Exhaust Temp | RH Exhaust | Current (SCT)
  - Row 2: Gauge Pressure | Raw Absolute Pressure
- HVAC layout unchanged (auto-fill behavior preserved).

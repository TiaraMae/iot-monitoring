# AGENTS.md — IoT Monitoring v4 (Production-Ready)

> **If you are an AI agent working on this folder (`iot_thesis_v4/`), read this file first.**

---

## 1. Project Identity

This is the **v4 iteration** of the IoT Monitoring & Predictive Maintenance thesis project.

v4 is functionally identical to v3 but has been hardened for production deployment and public repository sharing.
...
### Core Architecture
**The backend owns the calibration factor (CF) and deductor values.** When a sensor node boots or reconnects, the backend sends its CF and deductor via MQTT. The node computes current locally and sends the calculated `CurrentA` in telemetry.

### v4 Hardening
- **No hardcoded credentials:** `MQTT_HOST`, `MQTT_USER`, `MQTT_PASS` require environment variables. No fallback defaults.
- **Full TLS validation:** ESP32 firmware embeds ISRG Root X1 CA certificate (`setCACert()`) instead of `setInsecure()`.
- **Production deployment scripts:** Ubuntu setup, nginx reverse proxy, systemd service, firewall rules included.
- **Credential sanitization:** `_Clean` firmware folders with placeholders committed to GitHub; real versions excluded via `.gitignore`.
...
│ ├── login.html
│ └── signup.html
├── Update_SensorNode/ # Real firmware (local only, gitignored)
...
**Do not mix with `iot_thesis/` (v1), `iot_thesis_v2/` (v2), or `iot_thesis_v3/` (v3).** They are independent systems.
...
- **Full TLS validation:** `WiFiClientSecure` uses `setCACert(ISRG_Root_X1)` instead of `setInsecure()`. The ISRG Root X1 certificate is embedded in the firmware source.
- **Clean variant for GitHub:** `Update_SensorNode_Clean/Update_SensorNode_Clean.ino` contains placeholder credentials. Real version is excluded via `.gitignore`.

---

## 4. Backend Architecture (`app.py`)

### Environment-Only Configuration (v4)

v4 removes all hardcoded MQTT defaults from `app.py`:
...
**No fallback values.** The application will not start without a valid `.env` file.
...
### Snapshot Capture
- **Non-inverter:** Evaluated at **1 hour** of continuous running, then re-evaluated every additional hour. Final evaluation before turn-off if never evaluated.
- **Inverter:** Single evaluation at **15 minutes** into the high-effort window (T_return > 26.5°C). Uses the **last 6 readings (~1 minute)** at the 15-minute mark. Short cycles (< 15 min) are intentionally skipped — no early evaluation.
...
- **Timing:** All HVAC fault logic uses `actual_time` (sensor-corrected timestamp from telemetry) instead of server `now`. This ensures correct evaluation timing even when ESP32 buffers messages during offline periods.
...
## 10. Production Deployment (v4)

v4 includes a complete deployment suite in the `deployment/` directory:

### `setup-ubuntu.sh`
- Updates system packages
- Installs Python 3, pip, PostgreSQL, nginx, Certbot, Fail2ban
- Creates `iot` user and virtual environment
- Clones repository and installs requirements
...
- HTTPS (port 443) → proxies to Gunicorn on `unix:/run/iot-backend.sock`
...
- Binds to Unix socket `/run/iot-backend.sock`
- Auto-restart on failure
- Runs as `iot` user

### `setup-firewall.sh`
- UFW rules: allow 22 (SSH), 80 (HTTP), 443 (HTTPS), 8883 (MQTTS outbound)
- Default deny incoming

### `.env.production`
Template with placeholders. Must be filled with real credentials before deployment:
- `FLASK_SECRET_KEY`
...
- **New capability:** Users can now remove/clear a previously saved baseline via a ` Remove Baseline` button next to ` Edit Baseline`.
...
- **HVAC evaluation redesigned:**
 - Non-inverter: evaluates at **1 hour** of running, then every additional hour. Final evaluation before turn-off if never evaluated.
 - Inverter: single evaluation at **15 minutes** into high-effort window (T_return > 26.5°C). Short cycles (< 15 min) skipped.
 - Uses **average of last 6 readings** at the evaluation point (~1 minute smoothing window).
...
### 2026-05-26 — Inverter Evaluation Redesigned to 15-Minute Single-Shot
- **Inverter start threshold:** `Treturn > 26.5°C` (was 27°C).
- **Inverter evaluation timing:** Single evaluation at exactly **15 minutes** after cycle start. Removed periodic re-evaluation every 5 minutes.
- **Short cycles:** Cycles that end before 15 minutes are **intentionally skipped** — no early evaluation, no alert.
- **Re-evaluation parameter:** `_evaluate_if_ready()` now accepts `reevaluate=True/False`. Inverter passes `reevaluate=False`; non-inverter keeps `reevaluate=True`.
- **HVAC timing fix:** All HVAC fault logic (`start_time`, `runtime`, `created_at`) now uses `actual_time` (sensor-corrected timestamp from `reading_data['_actual_time']`) instead of server `now`. Prevents incorrect 15-minute countdown when ESP32 buffers offline messages and flushes them in a burst.

### 2026-05-26 — Dryer Alert Timestamp Bug Fix
- **Root cause:** `_finalize_dryer_cycle()` used `now` (timestamp of the reading that triggered cycle-end detection). For gap-triggered finalization, this `now` was actually the **first reading of the NEXT cycle**, so end-of-cycle alerts were timestamped with the next cycle's start time.
- **Fix:** Compute `cycle_end_time = stats.get('last_time', now)` at the top of `_finalize_dryer_cycle()`. Use `cycle_end_time` for all `_insert_fault_alert()` calls (lint blockage, incomplete drying, belt snap, roller wear).
- **v2 fix:** Same `cycle_end_time` fix in `_finalize_dryer_cycle()`. Also fixed gap-abort belt snap alert in `_check_dryer_faults()` to use `stats['last_time']`.

### 2026-05-26 — Emoji Removal
- **Scope:** Removed all emoji characters from `.py`, `.html`, `.ino`, and `.md` files across `iot_thesis/`, `iot_thesis_v2/`, `iot_thesis_v3/`, `gas_dryer_test/`, and root `README.md`.
- **Replaced:** Discord alert titles, button labels, Serial.println strings, UI icons.
- **Preserved:** All functional code unchanged. Leading/trailing spaces cleaned up. Stray emoji variation selectors (U+FE0F) also removed.
- **Deleted:** `iot_thesis_v3/emoji_scan.txt` (outdated scan report).

### 2026-05-26 — Gas Dryer Gauge Pressure Calculation
- **Objective:** Convert raw absolute BME280 pressure into gauge pressure (raw - atmospheric baseline) for display and analytics.
- **Atmospheric tracking:** EMA with alpha=0.3, updated every 2 minutes during idle. 15-second settling guard rejects readings too close to running cycles.
- **State machine:** IDLE -> EMA updates; RUNNING -> atmospheric locked; POST-RUN -> immediate hard reset on first idle reading.
- **DB migrations:** `ALTER TABLE appliances ADD COLUMN atmospheric_pressure REAL`; `ALTER TABLE dryer_readings ADD COLUMN abs_pressure REAL`.
- **Backend:** `DRYER_ATMOSPHERIC_TRACKER` global dict; `_update_atmospheric_pressure()` function; MQTT handler computes gauge = raw - atmospheric and stores gauge in `dryer_readings.pressure`, raw in `abs_pressure`.
- **API:** `api_device_latest`, `api_device_latest_n`, `get_table_data` now return `AbsPressure` alongside `Pressure` (gauge). All dryer SELECT queries include `abs_pressure`.
- **Analytics:** `dryer_analytics` computes `avg_pressure` (average gauge pressure) per cycle and includes it in the returned cycle JSON.
- **Dashboard:** Chart 3 renamed to "Exhaust Gauge Pressure". Detail label changed to "Gauge Pressure (hPa)". Live data shows "hPa (gauge)". Analytics table adds "Avg Gauge Pressure (hPa)" column immediately to the right of "End RH".
- **Note:** Existing pressure SPC baselines must be re-set after deployment, as idle gauge pressure is approximately 0 hPa (vs ~1013 hPa absolute).

### 2026-05-26 — Credential Sanitization for GitHub
- **Real firmware excluded from Git:** All `Update_SensorNode/` and `esp32dryertest/` folders with hardcoded credentials added to root `.gitignore`. Real versions stay on local machine only.
- **Clean versions created:** `_Clean` folders for all 6 firmware variants with placeholder credentials.
- **Backend sanitized:** Hardcoded MQTT defaults removed from `app.py`. All MQTT values require environment variables.
- **Deployment scripts sanitized:** `.env.production` and `setup-ubuntu.sh` use placeholder values.

### 2026-05-26 — Production Deployment (v4)
- **Architecture:** Self-hosted mini PC runs PostgreSQL + Flask (Gunicorn) + Nginx (HTTPS). HiveMQ Cloud retained for MQTTS broker.
- **ESP32 security fix:** Removed `espClient.setInsecure()` from firmware. Added embedded ISRG Root X1 CA certificate for proper TLS certificate validation against HiveMQ Cloud.
- **Backend production readiness:** Added `gunicorn` to `requirements.txt`. Created `deployment/` directory with:
  - `.env.production` template
  - `iot-backend.service` (systemd)
  - `nginx-iot-monitor.conf` (reverse proxy + HTTPS + rate limiting)
  - `setup-firewall.sh` (UFW rules: 22/80/443/8883)
  - `setup-ubuntu.sh` (full automated server setup)
- **Ports:** 22 (SSH), 80 (HTTP redirect), 443 (HTTPS dashboard), 8883 (MQTTS to HiveMQ), 5432 (PostgreSQL localhost only).
- **Security:** Let's Encrypt for HTTPS, Fail2ban for brute-force protection, UFW firewall, Nginx security headers, PostgreSQL bound to localhost only.


## 12. Testing Discord Alerts via Tight Baselines (No Code Changes)
...
- **Change:** All humidity `apply_calibration()` calls removed. `RHreturn`, `RHsupply`, `RHexhaust`, and `RHAmbient` now use **raw sensor values** throughout the system (dashboard, charts, exports, SPC, fault detection).
- **Reason:** Linear regression calibration for humidity was unreliable — operating conditions (high moisture loads, rapid temperature swings) frequently produced calibrated values outside the 0–100% physical range. After advisor consultation, raw values were adopted as the definitive approach.
- **Impact:**
 - Dashboard cards show raw RH% directly from DHT22/BME280.
 - Chart.js humidity lines use raw values.
 - Excel exports contain raw RH values.
 - SPC baselines for RH metrics are configured against raw values.
 - Dryer fault detection (`lint_blockage`, `incomplete_drying`) evaluates raw end-of-cycle RH.
 - `rhreturn_slope`, `rhreturn_intercept`, `rhsupply_slope`, `rhsupply_intercept` columns in `appliances` table are **deprecated and unused**.
